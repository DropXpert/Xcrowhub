-- Raise the minimum NIM escrow value while keeping the USDT minimum at 1.
-- Existing historical rows remain valid; new amounts and amount changes use
...
begin
  ...
  if new.price_currency = 'NIM' and new.price_amount < 500 then
    raise exception 'Minimum NIM deal amount is 500 NIM';
  end if;
...
begin
  if new.price_currency = 'NIM' and new.price_amount < 500 then
    raise exception 'Minimum NIM listing amount is 500 NIM';
  end if;
  if new.price_currency = 'USDT' and new.price_amount < 1 then
...
begin
  if new.currency = 'NIM'
     and (new.original_amount < 500 or new.current_amount < 500) then
    raise exception 'Minimum NIM offer amount is 500 NIM';
  end if;
