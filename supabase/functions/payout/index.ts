...
async function handleCampaignCoupon(
  body: PayoutRequest,
  auth: AuthContext,
): Promise<Response> {
  const claimId = body.campaign_coupon_claim_id;
  if (!claimId) return json({ error: "campaign_coupon_claim_id is required" }, 400);

  const { data: claim, error: claimError } = await supabase
    .from("campaign_coupon_claims")
    .select("id, coupon_code, wallet_address, wallet_norm, payout_status, payout_tx_hash")
    .eq("id", claimId)
    .maybeSingle();
  if (claimError) throw claimError;
  if (!claim) return json({ error: "Campaign coupon claim not found" }, 404);

  if (!auth.internal && !auth.admin && normalizedAddress(auth.walletAddr) !== normalizedAddress(claim.wallet_address)) {
    return json({ error: "Unauthorized" }, 401);
  }
  if (claim.payout_status === "paid" && claim.payout_tx_hash) {
    return json({ success: true, txHash: claim.payout_tx_hash, idempotent: true });
  }
  if (claim.payout_status !== "pending") {
    return json({ error: `Coupon payout is not pending (current: ${claim.payout_status})` }, 409);
  }

  const { data: coupon, error: couponError } = await supabase
    .from("campaign_coupons")
    .select("code, reward_nim, active")
    .eq("code", claim.coupon_code)
    .maybeSingle();
  if (couponError) throw couponError;
  if (!coupon || !coupon.active) return json({ error: "Coupon is inactive" }, 409);

  // Wallet auth normalizes NIM addresses to lowercase for matching. The Nimiq
  // signer accepts the canonical uppercase user-friendly form, so restore that
  // representation before sending the payout request.
  const recipient = String(claim.wallet_address ?? "").replace(/\s+/g, "").toUpperCase();
  if (!recipient) return json({ error: "Coupon claim has no recipient address" }, 409);
  if (isCustodyRecipient("NIM", recipient)) {
    return json({ error: "Coupon recipient cannot be the custody address" }, 409);
  }

  let units: bigint;
  try {
    units = decimalToUnits(String(coupon.reward_nim), 5);
  } catch (parseError) {
    return json({ error: (parseError as Error).message }, 409);
  }
  if (units <= 0n) return json({ error: "Coupon payout amount is not positive" }, 409);

  return await executePayout({
    payoutKey: `campaign-coupon:${claimId}`,
    subjectKind: "campaign_coupon",
    payoutKind: "campaign_coupon",
    dealId: null,
    referralClaimId: null,
    campaignCouponClaimId: claimId,
    network: "nimiq",
    currency: "NIM",
    recipient,
