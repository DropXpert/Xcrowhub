...
serve(async (req: Request) => {
  ...
  try {
    ...
    const verifiedBuyer = buyer || result.from || "";
    const chainSender = result.from ?? verifiedBuyer;
...
async function verifyNim(
  ...
  if (
    buyer
    && !addressesEqual(from, buyer)
    && !nimiqMemoMatches(tx.recipientData ?? tx.data ?? tx.extraData ?? "", dealId)
  ) {
    return { ok: false, reason: "sender mismatch and deal memo mismatch" };
  }
  return {
    ok: true,
    from,
...
// account returned during login. In that case, bind the transaction to the
// deal's exact memo instead of weakening recipient or amount verification.
// Some RPC providers expose recipientData as hex-encoded hex, so safely try at
// most two decoding passes.
