...
async function processTx(tx: any): Promise<boolean> {
  ...
  // Try to extract an exact deal ID from the Nimiq payment memo.
  let dealId: string | null = null;
  for (const decoded of decodeNimiqData(dataHex)) {
    const match = /^XcrowHub (PH-[A-Z0-9]{4}-[A-Z0-9]{4})$/.exec(decoded);
    if (match) {
      dealId = match[1];
      break;
    }
  ...
  if (dealId) {
    ...
    if (data) {
      ...
      if (
        ...
        || !hashAgrees
        || (
          data.buyer_wallet_address
          && !addressesEqual(data.buyer_wallet_address, fromAddr)
          && !nimiqMemoMatches(dataHex, data.id)
        )
      ) {
  ...
  if (!deal) {
    ...
    if (
      ...
      || (
        deal.buyer_wallet_address
        && !addressesEqual(deal.buyer_wallet_address, fromAddr)
        && !nimiqMemoMatches(dataHex, deal.id)
      )
    ) {
  ...
  const { data: confirmed, error: rpcErr } = await supabase.rpc("claim_and_confirm_deal_payment", {
    p_deal_id: deal.id,
    p_buyer: deal.buyer_wallet_address || fromAddr,
    p_tx_hash: txHash,
    p_network: "nimiq",
...
function decodeNimiqData(value: unknown): string[] {
  const decoded: string[] = [];
  let candidate = typeof value === "string" ? value.trim() : "";

  for (let pass = 0; pass < 3 && candidate; pass++) {
    decoded.push(candidate);
    if (!/^(?:0x)?[0-9a-f]+$/i.test(candidate)) break;

    const clean = candidate.replace(/^0x/i, "");
    if (clean.length % 2 !== 0) break;
    try {
      const bytes = new Uint8Array(clean.match(/.{2}/g)!.map((byte) => parseInt(byte, 16)));
      candidate = new TextDecoder("utf-8", { fatal: true }).decode(bytes).trim();
    } catch {
      break;
    }
  }
...
function nimiqMemoMatches(value: unknown, dealId: string): boolean {
  const expected = `XcrowHub ${dealId}`;
  return decodeNimiqData(value).some((candidate) => candidate === expected);
}
