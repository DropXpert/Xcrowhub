 ...
 *
 * Normal paths:
 * - the buyer can release the full deal to the fixed seller;
 * - the seller can refund the full deal to the fixed buyer.
 *
 * Disputed or split settlements require two distinct EIP-712 signatures from
 * the buyer, seller, and immutable arbitrator.
 */
contract XcrowHubEscrow is EIP712, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint16 public constant MAX_FEE_BPS = 100; // 1%

    bytes32 public constant SETTLEMENT_TYPEHASH = keccak256(
        "Settlement(bytes32 dealId,uint256 buyerAmount,uint256 sellerAmount,uint256 nonce,uint256 deadline)"
    );
    ...
    mapping(bytes32 dealId => Escrow escrow) public escrows;
    ...
    /**
     * @notice Locks an exact amount of USDT for one deal.
     * @dev The buyer must approve this contract before calling.
     */
    function fund(bytes32 dealId, address seller, uint256 amount, uint16 feeBps)
        external
        nonReentrant
    {
        if (dealId == bytes32(0)) revert InvalidSettlement();
        if (
            seller == address(0)
                || seller == msg.sender
                || seller == address(this)
        ...
        emit EscrowFunded(dealId, msg.sender, seller, amount, feeBps);
    }
    ...
    function release(bytes32 dealId) external nonReentrant {
        Escrow storage escrow = _fundedEscrow(dealId);
        if (msg.sender != escrow.buyer) revert Unauthorized();
        _finalize(dealId, escrow, 0, escrow.amount);
    }
    ...
    function refund(bytes32 dealId) external nonReentrant {
        ...
        _finalize(dealId, escrow, escrow.amount, 0);
    }
    ...
    function settle(
        ...
        _finalize(dealId, escrow, buyerAmount, sellerAmount);
    }
    ...
    function _fundedEscrow(bytes32 dealId)
        private
        view
        returns (Escrow storage escrow)
    {
        escrow = escrows[dealId];
