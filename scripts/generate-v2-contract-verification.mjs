...
const contractSource = "contracts/XcrowHubEscrowV2.sol";
const contractName = "XcrowHubEscrowV2";
const contractAddress = "0x639e0fB779e3D796cAC219850a26Ec3bDcE5d93c";
const deploymentTx =
  "0x5c4a9bbb83a9538f6a318ea2dd9ca1bd42fdf8956b70b4d7e25b3871af2bcb19";
...
const [transaction, receipt, deployedCode] = await Promise.all([
...
const immutableRanges = Object.values(
  compiled.evm.deployedBytecode.immutableReferences ?? {},
).flat();
const maskImmutables = (value) => {
  let body = value.replace(/^0x/, "").toLowerCase();
  for (const { start, length } of immutableRanges) {
    const offset = start * 2;
    body =
      body.slice(0, offset)
      + "0".repeat(length * 2)
      + body.slice(offset + length * 2);
  }
  return body;
};
if (
  maskImmutables(deployedCode)
  !== maskImmutables(`0x${compiled.evm.deployedBytecode.object}`)
) {
  throw new Error("Compiled runtime bytecode does not match the deployed contract");
}
...
fs.writeFileSync(
  path.join(
    outputDirectory,
    "XcrowHubEscrowV2-polygon-standard-input.json",
  ),
  `${JSON.stringify(input, null, 2)}\n`,
);
fs.writeFileSync(
  path.join(outputDirectory, "XcrowHubEscrowV2-constructor-arguments.txt"),
  `${constructorArgs}\n`,
);
...
console.log("runtime_bytecode_match=true");
