...
export function loadStoredSession(): AuthSession | null {
  try {
    ...
    // A JWT is signed by one specific backend. Reusing a persisted token after
    // a backend migration produces valid-looking sessions whose RLS requests
    // are rejected, leaving pages such as Deals silently empty.
    if (
      Date.now() > session.expiresAt
      || (isSupabaseConfiguredForClient() && session.backendUrl !== supabaseConfig.url)
    ) {
