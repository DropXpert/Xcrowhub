...
import { limitText } from "@/lib/inputLimits";
import { validateDealAmount } from "@/lib/dealAmounts";

function validAmount(value: string, currency: Currency): string {
  return String(validateDealAmount(value, currency));
}
...
export const useOfferStore = create<OfferState>((set, get) => {
  ...
  return {
    ...
    placeOffer: async (input) => {
      const now = nowIso();
      const amount = validAmount(input.amount, input.currency);
      const base: Offer = {
    ...
    counter: async (id, newAmount) => {
      const state = get();
      const offer = [
        ...Object.values(state.byListing).flat(),
        ...state.myOffers,
        ...state.incoming,
      ]
        .find((candidate) => candidate.id === id);
      if (!offer) throw new Error("Offer could not be found.");
      const amount = validAmount(newAmount, offer.currency);
