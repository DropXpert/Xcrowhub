...
import { minimumDealAmount } from "@/lib/dealAmounts";
...
export function OfferCard({
  ...
  const minimum = minimumDealAmount(offer.currency);
  const counterValid = Number.isFinite(numeric) && numeric >= minimum && numeric <= MAX_AMOUNT;
    ...
    <div className="rounded-xl border border-edge bg-surface px-4 py-3.5 space-y-3">
      ...
      {sellerActs && countering && (
        <div className="space-y-2">
          ...
          {!counterValid && counterAmount && (
            <p className="text-[12px] text-danger">
              Minimum counter is {minimum} {offer.currency}.
            </p>
          )}
