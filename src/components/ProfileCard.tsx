...
/* Ported from reactbits.dev/components/profile-card.
   CSS custom-property names carry a `--pc-` prefix here (not the upstream bare
   names) so they don't collide with anything else on the page. */
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const getOffsets = (
    evt: PointerEvent | ReactPointerEvent<Element>,
    el: HTMLElement
  ) => {
  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      ...
      // iOS 13+ gates DeviceMotion behind a permission grant.
      const anyMotion = (
