...
import { minimumDealAmount, validateDealAmount } from "@/lib/dealAmounts";
...
export default function CreateDeal() {
  ...
  function submit(e: React.FormEvent) {
    ...
    try {
      validateDealAmount(form.priceAmount, form.priceCurrency);
    } catch (err) {
          ...
          <Field label="Price" required>
            <div className="space-y-2">
              <input
                ...
                type="number"
                inputMode="decimal"
                min={minimumDealAmount(form.priceCurrency)}
                max="1000000000"
                step="0.01"
                placeholder={String(minimumDealAmount(form.priceCurrency))}
                value={form.priceAmount}
                onChange={(e) => update("priceAmount", e.target.value)}
              />
              {form.priceCurrency === "NIM" && (
                <NimUsdtEstimate nimAmount={form.priceAmount} />
              )}
            </div>
          </Field>
