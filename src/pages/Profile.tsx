...
export default function Profile() {
  ...
  const claimingCoupon = useCampaignStore((s) => s.claimingCoupon);
  ...
  async function handleCouponClaim() {
    const code = couponCode.trim();
    if (!code || claimingCoupon) return;
    const result = await claimCoupon(code);
    if (result?.payoutStatus === "paid") setCouponCode("");
  }
  ...
  return (
    <div className="space-y-4 lg:mx-auto lg:max-w-4xl">
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[13px] text-muted">
              Share your personal link and track eligible referral activity.
            </p>
          </div>
      ...
      {/* Campaign coupon (own profile only) */}
      {isOwn && (
        <section className="card space-y-3 px-5 py-4" data-tour="profile-coupon">
          <div className="flex items-start gap-3">
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-accent-soft text-accent">
              <Ticket className="h-[18px] w-[18px]" />
            </span>
            <div className="min-w-0">
              <h3 className="text-[14px] font-semibold text-ink">Campaign coupon</h3>
              <p className="mt-0.5 text-[12.5px] leading-relaxed text-muted">
                Enter an official XcrowHub campaign coupon. One claim per wallet.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") void handleCouponClaim();
              }}
            />
            <button
              type="button"
              ...
              onClick={() => void handleCouponClaim()}
            >
              {claimingCoupon ? "Claiming..." : "Claim"}
            </button>
          </div>
          {couponResult && (
            <p className="rounded-md border border-accent/30 bg-accent-soft px-3 py-2 text-[12.5px] text-accent-ink">
              {couponResult.alreadyClaimed
                ? couponResult.payoutStatus === "paid"
                  ? `${couponResult.amount} NIM campaign coupon was already paid to this wallet.`
                  : `Coupon claimed, but the ${couponResult.amount} NIM payout is still pending. Try again shortly.`
                : couponResult.payoutStatus === "paid"
                  ? `${couponResult.amount} NIM sent to your wallet successfully.`
                  : `Coupon claimed, but the ${couponResult.amount} NIM payout is still pending. Try again shortly.`}
            </p>
          )}
          {couponError && !couponResult && <p className="text-[12.5px] text-danger">{couponError}</p>}
        </section>
      )}
